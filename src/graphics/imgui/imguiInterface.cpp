    ...
    void ImguiInterface::gameObjectsBar(std::vector<::Pixel::GameObject>& gameObjects, int &selectedGameObjectIndex, const projects::Project& currentProject)
    {
        ...
        inspectorBar.Draw([&]() {
            ...
            if (!selected.hasComponent<ecs::components::Script>()) {
                ...
                ImGui::SameLine();
                bool open = ImGui::CollapsingHeader("Script Component", nullptr, ImGuiTreeNodeFlags_DefaultOpen);
                ...
                if (open) {
                    ...
                    std::strncpy(scriptPathBuffer, s->scriptPath.c_str(), sizeof(scriptPathBuffer) - 1);
                ...
                for (const auto& comp : availableComponents) {
                    ...
                    if (ImGui::Selectable(comp.c_str())) {
                        if (comp == "Sprite") {
                            ...
                            availableComponents.erase(std::remove(availableComponents.begin(), availableComponents.end(), "Script"), availableComponents.end());
                        }
                        query.clear();
                        search[0] = '\0';
                    }
