...
namespace Pixel
{
    ...
    static constexpr int CHUNKS_SIZE = 32;
    ...
    class Chunk
    {
        public:
             ...
             * @param x The local x-coordinate (0 to CHUNKS_SIZE-1) within the chunk.
             * @param y The local y-coordinate (0 to CHUNKS_SIZE-1) within the chunk.
             * @return The PixelEntityID at the specified coordinates, or EMPTY if out of bounds
             */
            PixelEntityID get(int x, int y) const {
                if (x < 0 || x >= CHUNKS_SIZE || y < 0 || y >= CHUNKS_SIZE) {
                    return EMPTY;
                }
                return cells[x][y];
            }
        ...
        public:
            ...
            */
            PixelEntityID getPixel(int x, int y) {
                int cx = floor((float)x / CHUNKS_SIZE);
                int cy = floor((float)y / CHUNKS_SIZE);
                int lx = (x % CHUNKS_SIZE + CHUNKS_SIZE) % CHUNKS_SIZE;
                int ly = (y % CHUNKS_SIZE + CHUNKS_SIZE) % CHUNKS_SIZE;
            ...
            */
            ...
            bool movePixel(int oldX, int oldY, int newX, int newY) {
                ...
                int cx = floor((float)newX / CHUNKS_SIZE);
                int cy = floor((float)newY / CHUNKS_SIZE);
                int lx = (newX % CHUNKS_SIZE + CHUNKS_SIZE) % CHUNKS_SIZE;
                int ly = (newY % CHUNKS_SIZE + CHUNKS_SIZE) % CHUNKS_SIZE;
