    ...
    private:
        ...
        void registerBindings()
        {
            ...
            lua.set_function("is_key_pressed", [](const std::string &key) -> bool {
                ...
                if (key == "up"    || key == "w") return state[SDL_SCANCODE_W];
                if (key == "down"  || key == "s") return state[SDL_SCANCODE_S];
                if (key == "left"  || key == "a") return state[SDL_SCANCODE_A];
                if (key == "right" || key == "d") return state[SDL_SCANCODE_D];
                if (key == "space")               return state[SDL_SCANCODE_SPACE];
                if (key == "lshift")              return state[SDL_SCANCODE_LSHIFT];
                if (key == "escape")              return state[SDL_SCANCODE_ESCAPE];
                return false;
            });
