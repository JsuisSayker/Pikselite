...
namespace ecs::systems
{
    ...
    public:
        ...
        void init() override
        {
            if (_eventBus)
            {
                _collisionEnterHandlerId = _eventBus->subscribe<engine::events::CollisionEnterEvent>(
        ...
        bool loadScript(const std::string& /*filepath*/) { return true; }
    ...
    private:
        ...
        bool ensureScriptLoaded(ecs::EntityID entity,
                                ...
                                ScriptInstance& instance)
        {
            if (!instance.lua || instance.scriptPath != scriptPath)
            {
                ...
                {
                    std::cerr << "[ScriptSystem] Failed to load script for entity "
                              << entity << ": " << scriptPath << std::endl;
                    return false;
                }
            }
        ...
        void registerBindings(engine::LuaManager& luaManager)
        {
            ...
            lua.set_function("is_key_pressed", [](const std::string& key) -> bool {
                const std::string lowerKey = toLower(key);
                const Uint8* state = SDL_GetKeyboardState(nullptr);
                if (lowerKey == "up" || lowerKey == "w") return state[SDL_SCANCODE_W];
                if (lowerKey == "down" || lowerKey == "s") return state[SDL_SCANCODE_S];
                if (lowerKey == "left" || lowerKey == "a") return state[SDL_SCANCODE_A];
                if (lowerKey == "right" || lowerKey == "d") return state[SDL_SCANCODE_D];
            ...
            lua.set_function("get_entity_by_name", [this](const std::string& name, sol::this_state ts) -> sol::object {
                sol::state_view view(ts);
                ...
                for (const auto& entityPtr : _entityManager->getEntities())
                {
                    ...
                    {
                        return sol::make_object(view, buildEntityTable(view, entityId));
                    }
                }
            ...
            lua.set_function("get_entities", [this](sol::this_state ts) -> sol::table {
                sol::state_view view(ts);
            ...
            lua.set_function("create_entity", sol::overload(
                [this]() -> std::uint32_t { return createEntityFromTable(sol::optional<sol::table>()); },
                [this](sol::table data) -> std::uint32_t { return createEntityFromTable(data); }));

            lua.set_function("delete_entity", [this](std::uint32_t id) -> bool {
                return deleteEntity(id);
            });

            lua.set_function("has_component", [this](std::uint32_t id, const std::string& type) -> bool {
                return hasComponent(id, type);
            });

            lua.set_function("get_component", [this](std::uint32_t id,
            ...
            lua.set_function("set_component", [this](std::uint32_t id,
                                                      const std::string& type,
                                                      sol::table data) -> bool {
                return setComponentFromTable(id, type, data);
            });

            lua.set_function("add_component", [this](std::uint32_t id,
                                                      const std::string& type,
                                                      sol::table data) -> bool {
                return addComponentFromTable(id, type, data);
            });

            lua.set_function("remove_component", [this](std::uint32_t id,
                                                         const std::string& type) -> bool {
                return removeComponent(id, type);
            });

            lua.set_function("set_victory", [this](const std::string& reason) {
                if (_victory)
                    return;
                _victory = true;
            ...
            lua.set_function("set_lose", [this](const std::string& reason) {
                if (_lose)
                    return;
                _lose = true;
                _loseReason = reason;
            ...
            lua.set_function("create_pixels", [this](sol::table entries) -> int {
                int accepted = 0;
            ...
            lua.set_function("basic_chase", [this](std::uint32_t selfId,
                                                    std::uint32_t targetId,
                                                    float speed,
                                                    sol::this_state ts) -> bool {
                if (!_activeComponentManager)
                    return false;
                if (!_entityManager || !_entityManager->hasEntity(selfId) || !_entityManager->hasEntity(targetId))
                    return false;
                ...
                auto& selfTransform = _activeComponentManager->getComponent<components::Transform>(selfId);
                const auto& targetTransform = _activeComponentManager->getComponent<components::Transform>(targetId);
                auto& selfVelocity = _activeComponentManager->getComponent<components::Velocity>(selfId);
        ...
        std::uint32_t createEntityFromTable(const sol::optional<sol::table>& data)
        {
            ...
            if (data)
            {
                ...
                if (physicsObj.valid() && physicsObj.is<sol::table>())
                {
                    ...
                    const std::string bodyType = toLower(p.get_or("bodyType", std::string("dynamic")));
                    if (bodyType == "static") physics.bodyType = b2_staticBody;
                    else if (bodyType == "kinematic") physics.bodyType = b2_kinematicBody;
                    else physics.bodyType = b2_dynamicBody;
        ...
        bool hasComponent(ecs::EntityID entityId, const std::string& type) const
        {
            ...
            if (key == "transform") return _activeComponentManager->hasComponent<components::Transform>(entityId);
            if (key == "velocity") return _activeComponentManager->hasComponent<components::Velocity>(entityId);
            if (key == "sprite") return _activeComponentManager->hasComponent<components::Sprite>(entityId);
            if (key == "physics" || key == "physicsbody") return _activeComponentManager->hasComponent<components::PhysicsBody>(entityId);
            if (key == "script") return _activeComponentManager->hasComponent<components::Script>(entityId);
            if (key == "name") return _entityNames.find(entityId) != _entityNames.end();
            return false;
        }

        sol::object getComponentAsObject(sol::state_view lua, ecs::EntityID entityId, const std::string& type)
        {
            ...
            if (key == "transform")
            {
                const auto& t = _activeComponentManager->getComponent<components::Transform>(entityId);
            ...
            {
                const auto& v = _activeComponentManager->getComponent<components::Velocity>(entityId);
            ...
            {
                const auto& p = _activeComponentManager->getComponent<components::PhysicsBody>(entityId);
                table["enabled"] = p.enabled;
                table["bodyType"] = p.bodyType == b2_staticBody ? "static" : (p.bodyType == b2_kinematicBody ? "kinematic" : "dynamic");
        ...
        bool setComponentFromTable(ecs::EntityID entityId, const std::string& type, const sol::table& data)
        {
            if (!hasComponent(entityId, type))
                return false;
            ...
            if (key == "physics" || key == "physicsbody")
            {
                ...
                const std::string bodyType = toLower(data.get_or("bodyType", std::string("dynamic")));
                if (bodyType == "static") p.bodyType = b2_staticBody;
                else if (bodyType == "kinematic") p.bodyType = b2_kinematicBody;
                else p.bodyType = b2_dynamicBody;
        ...
        bool addComponentFromTable(ecs::EntityID entityId, const std::string& type, const sol::table& data)
        {
            if (!_entityManager || !_activeComponentManager || !_entityManager->hasEntity(entityId))
                return false;
        ...
        bool removeComponent(ecs::EntityID entityId, const std::string& type)
        {
            ...
            const std::string key = toLower(type);
            if (key == "transform") { _activeComponentManager->removeComponent<components::Transform>(entityId); return true; }
            if (key == "velocity") { _activeComponentManager->removeComponent<components::Velocity>(entityId); return true; }
            if (key == "sprite") { _activeComponentManager->removeComponent<components::Sprite>(entityId); return true; }
            if (key == "physics" || key == "physicsbody") { _activeComponentManager->removeComponent<components::PhysicsBody>(entityId); return true; }
            if (key == "script")
            {
            ...
            if (key == "name") { _entityNames.erase(entityId); return true; }
        ...
        sol::table buildEntityTable(sol::state_view lua, ecs::EntityID entityId)
        {
            ...
            {
                const auto& transform = _activeComponentManager->getComponent<components::Transform>(entityId);
                table["x"] = transform.x;
                table["y"] = transform.y;
            ...
            {
                const auto& velocity = _activeComponentManager->getComponent<components::Velocity>(entityId);
            ...
            {
                const auto& script = _activeComponentManager->getComponent<components::Script>(entityId);
        ...
        void applyEntityTableChanges(ecs::EntityID entityId, const sol::table& table)
        {
            ...
            if (_activeComponentManager->hasComponent<components::Transform>(entityId))
            {
                auto& transform = _activeComponentManager->getComponent<components::Transform>(entityId);
            ...
            if (_activeComponentManager->hasComponent<components::Velocity>(entityId))
            {
                auto& velocity = _activeComponentManager->getComponent<components::Velocity>(entityId);
        ...
        void dispatchCollisionCallbacks(ecs::EntityID entityId, ScriptInstance& instance)
        {
            ...
            if (enterIt != _pendingCollisionEnter.end() && instance.lua->hasFunction("on_collision_enter"))
            {
            ...
            if (exitIt != _pendingCollisionExit.end() && instance.lua->hasFunction("on_collision_exit"))
            {
