/**
 * @file bars.hpp
 * @brief Declaration of the Bar class for rendering customizable UI bars (toolbars and sidebars)
 * using ImGui. This file defines the Bar class and its configuration structure, which are used to
 * create and
 */
#pragma once
...
namespace graphics
{
    ...
    /**
     * @brief Configuration structure for a UI bar, containing properties such as orientation,
     * label, size, visibility, and position. This structure is used to initialize and configure
     * instances of the Bar class.
     */
    struct BarConfig
    {
    ...
    /**
     * @brief Class for creating and managing customizable UI bars (toolbars and sidebars) using
     * ImGui.
     */
    class Bar
    {
    ...
    {
        if (position == "right")
            return ImVec2(-1.0f, 0.0f); // x<0 → right-anchored
        if (position == "bottom")
            return ImVec2(0.0f, -1.0f); // y<0 → bottom-anchored
    ...
    {
        if (slot == "full")
            return 0; // Bar::Draw ignores stored size and computes
        return 0;
    }
