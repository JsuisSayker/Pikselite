...
def list_all_issues(owner, repo, per_page=100):
    query = """
    query ($owner: String!, $name: String!, $per_page: Int!, $after: String) {
      repository(owner: $owner, name: $name) {
        issues(first: $per_page, after: $after, orderBy: {field: CREATED_AT, direction: ASC}) {
          nodes {
            number
            title
            body
...
if __name__ == "__main__":
    # replicate_github_issues()
    print("toto")
    issues = list_all_issues("Pikselite", "Pikselite-Engine")
